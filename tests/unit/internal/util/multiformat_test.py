...
from unit.helpers.serialization_helper import (
    SerializableFixture,
    DeserializableFixture,
    MultiformatModelFixture,
)
...
def complex_data():
    ...
    return {
        ...
        "string": "string",
        "number": 123,
        "nested": {"list": [Path("/nested/path"), SampleEnum.VALUE2]},
    }
...
def test_to_toml(complex_data):
    ...
    with mock.patch(
        "project_resolution_engine.internal.util.multiformat.dump_toml_to_str"
    ) as mock_dump:
        test_obj.to_toml()
...
def test_serialize(fmt, expected_method, error, branches):
    ...
    with (
        mock.patch.object(test_obj, "to_json", return_value="json_result") as mock_json,
...
def test_str_method():
    ...
    with mock.patch.object(
        test_obj, "flat_summary", return_value="flat_summary_result"
    ) as mock_summary:
        result = str(test_obj)
...
def test_deserialize():
    ...
    with (
        mock.patch.object(
            test_class, "_parse_text", return_value="parsed_data"
        ) as mock_parse,
        mock.patch.object(
            test_class, "_coerce_root_mapping", return_value={"key": "value"}
        ) as mock_coerce,
        mock.patch.object(
            test_class,
            "_preprocess_mapping",
            return_value={"processed_key": "processed_value"},
        ) as mock_preprocess,
        mock.patch.object(
            test_class,
            "from_mapping",
            return_value=DeserializableFixture({"from_mapping": "result"}),
        ) as mock_from_mapping,
        mock.patch.object(
            test_class, "_postprocess_instance", return_value="final_result"
        ) as mock_postprocess,
    ):
...
def test_format_specific_deserialize_methods(method, fmt, branches):
    ...
    with mock.patch.object(
        DeserializableFixture, "deserialize", return_value="deserialized_result"
    ) as mock_deserialize:
...
def test_from_file():
    ...
    with (
        mock.patch.object(
            test_class, "_load_text", return_value="file_content"
        ) as mock_load,
        mock.patch.object(
            test_class, "_infer_format_from_suffix", return_value="inferred_format"
        ) as mock_infer,
        mock.patch.object(
            test_class, "_parse_text", return_value="parsed_data"
        ) as mock_parse,
        ...
        mock_parse.assert_called_with(
            "file_content", fmt="explicit_format", path=mock.ANY
        )
        mock_coerce.assert_called_with(
            "parsed_data", fmt="explicit_format", path=mock.ANY
        )
        mock_preprocess.assert_called_with(
            {"key": "value"}, fmt="explicit_format", path=mock.ANY
        )
        mock_from_mapping.assert_called_with({"processed_key": "processed_value"})
        mock_postprocess.assert_called_with(
            mock.ANY, fmt="explicit_format", path=mock.ANY
        )
        ...
        mock_parse.assert_called_with(
            "file_content", fmt="inferred_format", path=mock.ANY
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "key: value",
            None,
            lambda: mock.patch.dict("sys.modules", {"yaml": None})
            and mock.patch("builtins.__import__", side_effect=ImportError),
            RuntimeError,
            ["B61", "B62"],
        ),
        ...
        (
            ...
            {"key": "value"},
            lambda: mock.patch(
                "project_resolution_engine.internal.util.multiformat.load_toml_text",
        ...
        # Unrecognized format
        ("unknown", "content", None, lambda: mock.MagicMock(), ValueError, ["B67"]),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # No attribute
        (False, None, True, lambda inst: None, None, ["B77"]),
        # Has attribute, setattr raises exception
        (
            True,
            None,
            True,
            lambda inst: mock.patch.object(
                inst,
                "source_description",
...
def test_postprocess_instance_exception():
    ...
    class ExceptionRaisingDeserializable(MultiformatDeserializableMixin):
        ...
        def source_description(self, value):
            if (
                value and ":" in value
            ):  # Only raise when setting a formatted value like "json:path"
...
def test_model_mixin_functionality():
    ...
    with (
        mock.patch.object(
            MultiformatModelFixture, "_parse_text", return_value=test_data
        ),
        mock.patch.object(
            MultiformatModelFixture, "_coerce_root_mapping", return_value=test_data
        ),
        mock.patch.object(
            MultiformatModelFixture, "_preprocess_mapping", return_value=test_data
        ),
    ):
